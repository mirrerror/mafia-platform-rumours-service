...
public class ServiceRegistryClientTests : IDisposable
{
    ...
    public async Task SendHeartbeatAsync_Succeeds_WhenApiCallIsSuccessful()
    {
        ...
        _mockHttpMessageHandler.Protected()
            .Setup<Task<HttpResponseMessage>>(
                "SendAsync",
                ItExpr.Is<HttpRequestMessage>(m => m.Method == HttpMethod.Post && m.RequestUri!.ToString().Contains("heartbeat")),
                ItExpr.IsAny<CancellationToken>()
            )
            .ReturnsAsync(new HttpResponseMessage(HttpStatusCode.OK));
    ...
    [Fact]
    public async Task SendHeartbeatAsync_Handles_ExceptionDuringHeartbeat()
    {
        Environment.SetEnvironmentVariable("DISCOVERY_SERVICE_URL", TestDiscoveryUrl);
        var client = CreateClient();
        await RegisterClientAsync(client);
        var testException = new HttpRequestException("Network error during heartbeat");
